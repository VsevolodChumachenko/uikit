...
function filterOptionsWithCorrection(
  options,
  search,
  label,
  customLabel,
  ...
  if (!filterCorrection) {
    return filterOptions(options, search, label, customLabel);
  }
  let search2 = search;
  filterCorrection.forEach((item) => {
    search2 = search2.replace(item.key, item.value).toLowerCase();
  });
  return options.filter((option) => {
    return (
      includes(customLabel(option, label), search) ||
      includes(customLabel(option, label), search2)
    );
  });
}
...
export default {
  ...
  props: {
    ...
    /**
     * Put item value to search on Select
    ...
    filteredOptions() {
      ...
      if (this.internalSearch) {
        options = this.groupValues
          ...
          : this.filterCorrection
          ? filterOptionsWithCorrection(
              options,
              normalizedSearch,
              this.label,
              this.customLabel,
  ...
  watch: {
    internalValue() {
      ...
      if (!this.resetAfter && this.internalValue.length && !this.multiple) {
        this.search = this.internalValue[0].text;
      }
    },
    search() {
      if (this.pattern && this.search) {
        this.search = this.search.replace(new RegExp(this.pattern, 'g'), '');
      }
    ...
    searchValue() {
      if (this.putItemValueOnSelect) this.search = this.searchValue;
    },
  },
  methods: {
    /**
    ...
    select(option, key) {
      ...
      if (option.isTag) {
        ...
        /* istanbul ignore else */
        if (this.clearOnSelect && !this.putItemValueOnSelect) this.search = '';
      }
      /* istanbul ignore else */
